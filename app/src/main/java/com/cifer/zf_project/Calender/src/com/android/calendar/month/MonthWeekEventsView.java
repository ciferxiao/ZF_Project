...
package com.cifer.zf_project.Calender.src.com.android.calendar.month;
...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    protected Paint mWeekNumTodayCirclePaint;
    ...
    //xiao add
    private int oldMonthBackgroundColor;
    private Paint mTodayCirclePaint;
    private Paint mMonthNumTodayCirclePaint;
    private int mMonthEventBackgroundColor;
    ...
    public void createDna(ArrayList<Event> unsortedEvents) {
        ...
        if (!mShowDetailsInMonth) {
            ...
            mDNAAllDayPaint.setStrokeWidth(DNA_WIDTH);
    ...
    protected void loadColors(Context context) {
        ...
        mMonthNumTodayColor = res.getColor(R.color.month_week_num_color);
        ...
        mMonthDeclinedEventColor = res.getColor(R.color.agenda_item_declined_color);//no
        mMonthDeclinedExtrasColor = res.getColor(R.color.agenda_item_where_declined_text_color);//no
        mMonthEventExtraColor = res.getColor(R.color.month_event_extra_color);//no 
        ...
        oldMonthBackgroundColor =  res.getColor(R.color.white);
        mMonthEventBackgroundColor = res.getColor(R.color.month_event_background);
    }
    ...
    protected void initView() {
        ...
        mMonthNumTodayCirclePaint = new Paint();
        mMonthNumTodayCirclePaint.setFakeBoldText(false);
        mMonthNumTodayCirclePaint.setAntiAlias(true);
        mMonthNumTodayCirclePaint.setTextSize(TEXT_SIZE_MONTH_NUMBER);
        mMonthNumTodayCirclePaint.setColor(mMonthNumColor);
        mMonthNumTodayCirclePaint.setStyle(Style.FILL);
        mMonthNumTodayCirclePaint.setTextAlign(Align.CENTER);
        mMonthNumTodayCirclePaint.setTypeface(Typeface.DEFAULT);

        mMonthNumAscentHeight = (int) (-mMonthNumTodayCirclePaint.ascent() + 0.5f);
        mMonthNumHeight = (int) (mMonthNumTodayCirclePaint.descent() - mMonthNumTodayCirclePaint.ascent() + 0.5f);
        ...
        mEventExtrasPaint.setFakeBoldText(false);//加粗
        ...
        mEventExtrasPaint.setTextAlign(Align.RIGHT);
        ...
        mEventDeclinedExtrasPaint.setTextAlign(Align.CENTER);

        mWeekNumTodayCirclePaint = new Paint();
        mWeekNumTodayCirclePaint.setFakeBoldText(false);
        mWeekNumTodayCirclePaint.setAntiAlias(true);
        mWeekNumTodayCirclePaint.setTextSize(TEXT_SIZE_WEEK_NUM);
        mWeekNumTodayCirclePaint.setColor(mWeekNumColor);
        mWeekNumTodayCirclePaint.setStyle(Style.FILL);
        mWeekNumTodayCirclePaint.setTextAlign(Align.RIGHT);

        mWeekNumAscentHeight = (int) (-mWeekNumTodayCirclePaint.ascent() + 0.5f);
        ...
        mDNAAllDayPaint.setColor(mMonthEventBackgroundColor);
        ...
        mTodayCirclePaint.setColor(Color.BLUE);
        mTodayCirclePaint.setStyle(Style.STROKE);
        mTodayCirclePaint.setStrokeWidth(3); 

        if (DEBUG_LAYOUT) {
            Log.d("EXTRA", "mScale=" + mScale);
            Log.d("EXTRA", "mMonthNumTodayCirclePaint ascent=" + mMonthNumTodayCirclePaint.ascent()
                    + " descent=" + mMonthNumTodayCirclePaint.descent() + " int height=" + mMonthNumHeight);
            ...
            Log.d("EXTRA", "mWeekNumTodayCirclePaint ascent=" + mWeekNumTodayCirclePaint.ascent()
                    + " descent=" + mWeekNumTodayCirclePaint.descent());
        }
    }
    ...
    protected void drawDaySeparators(Canvas canvas) {
        ...
        p.setColor(oldMonthBackgroundColor);
    ...
    protected void drawBackground(Canvas canvas) {
        ...
        if (mHasToday) {
            p.setColor(oldMonthBackgroundColor);//xiao 今天的背景色
    ...
    protected void drawWeekNums(Canvas canvas) {
        ...
        if (mShowWeekNum) {
            ...
            canvas.drawText(mDayNumbers[0], x, y, mWeekNumTodayCirclePaint);
            numCount++;
            i++;
            todayIndex++;
            offset++;
        ...
        mMonthNumTodayCirclePaint.setColor(isFocusMonth ? mMonthNumColor : mMonthNumOtherColor);
        for (; i < numCount; i++) {
            if (mHasToday && todayIndex == i) {
                mMonthNumTodayCirclePaint.setColor(mMonthNumTodayColor);
                //xiao add for today circle
                //mMonthNumTodayCirclePaint.setFakeBoldText(isBold = true);
                if (i + 1 < numCount) {
                ...
                canvas.drawText(mDayNumbers[i], x - 21, y +15, mMonthNumTodayCirclePaint);
                canvas.drawCircle(x - 23,y+3,26,mTodayCirclePaint);
            ...
            x = computeDayLeftPosition(i - offset) - (SIDE_PADDING_MONTH_NUMBER);
            //xiao
            mMonthNumTodayCirclePaint.setTextSize(38); //day of months textsize
            canvas.drawText(mDayNumbers[i], x - 22, y +15, mMonthNumTodayCirclePaint);//xiao day of months position 
            //today 加粗屏蔽
            //if (isBold) {
            //    mMonthNumTodayCirclePaint.setFakeBoldText(isBold = false);
            //}
        }
    }
    ...
    protected int drawEvent(Canvas canvas, Event event, int x, int y, int rightEdge,
        ...
        Log.d("xiao111","textPaint == "+ text.toString());
    ...
    protected void drawDNA(Canvas canvas) {
        ...
        if (mDna != null) {
            for (Utils.DNAStrand strand : mDna.values()) {
                //android.util.Log.d("xiao111","strand == " + strand);
                //android.util.Log.d("xiao111","mDna.values() == " + mDna.values());
                if (strand.color == CONFLICT_COLOR || strand.points == null
                        || strand.points.length == 0) {
                    continue;
                }
                // xiao add agenda day background
                mDNATimePaint.setColor(mMonthEventBackgroundColor);
                //android.util.Log.d("xiao111","strand.points[0] ==" + strand.points[0]);
                //android.util.Log.d("xiao111","strand.points[1] ==" + strand.points[1]);
                //android.util.Log.d("xiao111","strand.points[2] ==" + strand.points[2]);
                //android.util.Log.d("xiao111","strand.points[3] ==" + strand.points[3]);
                canvas.drawCircle(strand.points[0] +50,strand.points[1] +30 ,20,mDNATimePaint);
                if(strand.points.length==8){
                    canvas.drawCircle(strand.points[4] +50,strand.points[5] +30 ,20,mDNATimePaint);
                }else if(strand.points.length==12){
                    canvas.drawCircle(strand.points[4] +50,strand.points[5] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[8] +50,strand.points[9] +30 ,20,mDNATimePaint);
                }else if(strand.points.length==16){
                    canvas.drawCircle(strand.points[4] +50,strand.points[5] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[8] +50,strand.points[9] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[12] +50,strand.points[13] +30 ,20,mDNATimePaint);
                }else if(strand.points.length==20){
                    canvas.drawCircle(strand.points[4] +50,strand.points[5] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[8] +50,strand.points[9] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[12] +50,strand.points[13] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[16] +50,strand.points[17] +30 ,20,mDNATimePaint);
                }else if(strand.points.length==24){
                    canvas.drawCircle(strand.points[4] +50,strand.points[5] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[8] +50,strand.points[9] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[12] +50,strand.points[13] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[16] +50,strand.points[17] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[20] +50,strand.points[21] +30 ,20,mDNATimePaint);
                }else if(strand.points.length==28){
                    canvas.drawCircle(strand.points[4] +50,strand.points[5] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[8] +50,strand.points[9] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[12] +50,strand.points[13] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[16] +50,strand.points[17] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[20] +50,strand.points[21] +30 ,20,mDNATimePaint);
                    canvas.drawCircle(strand.points[24] +50,strand.points[25] +30 ,20,mDNATimePaint);
                }                    
            ...
            if (strand != null && strand.points != null && strand.points.length != 0) {
                mDNATimePaint.setColor(Color.BLUE);
                canvas.drawLines(strand.points, mDNATimePaint);
            }
            if (mDayXs == null) {
                return;
            ...
            if (strand != null && strand.allDays != null && strand.allDays.length == numDays) {
                for (int i = 0; i < numDays; i++) {
                    ...
                    if (strand.allDays[i] != 0) {
                        //all day drawcircle
                        mDNAAllDayPaint.setColor(mMonthEventBackgroundColor);
                        canvas.drawCircle(mDayXs[i] + xOffset+21,DNA_ALL_DAY_HEIGHT+42,20,mDNAAllDayPaint);
