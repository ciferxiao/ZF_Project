...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    */
    ...
    public DayView(Context context, CalendarController controller,
        ...
        MIN_HOURS_WIDTH = (int) mResources.getDimension(R.dimen.min_hours_width); // xiao 时间列宽度
        ...
        MULTI_DAY_HEADER_HEIGHT = (int) mResources.getDimension(R.dimen.day_header_height);//xiao week header height
        int eventTextSizeId;
        if (mNumDays == 1) {
        ...
        mCurrentTimeLine = mResources.getDrawable(R.drawable.timeline_indicator_holo_light);// xiao add 分割线
        ...
        mNewEventHintColor =  mResources.getColor(R.color.white);//xiao text
    ...
    private void init(Context context) {
        ...
        mBgColor = mResources.getColor(R.color.white);//xiao header background color
        mCalendarAmPmLabel = mResources.getColor(R.color.calendar_ampm_label);
        mCalendarGridAreaSelected = mResources.getColor(R.color.mediumpurple);//xiao 点击颜色或选中框的颜色
        mCalendarGridLineInnerHorizontalColor = mResources
                .getColor(R.color.calendar_grid_line_inner_horizontal_color);// xiao 分割线颜色
        ...
        mTodayCircleColor = mResources.getColor(R.color.month_today_circle);       
        
        mEventTextPaint.setTextSize(EVENT_TEXT_FONT_SIZE);
        mEventTextPaint.setTextAlign(Align.LEFT);
        mEventTextPaint.setAntiAlias(true);

        int gridLineColor = mResources.getColor(R.color.colorAccent);
    ...
    public void handleOnResume() {
        ...
        mHourTimes = mIs24HourFormat ? getResources().getStringArray(R.array.istwentyfourhours) : getResources().getStringArray(R.array.istwelvehours);
        Log.d("xiao111","mHourTimes ==" + mHourTimes);
...
//        mContext.startProgressSpinner();
    ...
    private void drawAmPm(Canvas canvas, Paint p) {
        ...
        canvas.drawText(text, HOURS_LEFT_MARGIN + 25, y + 15, p);
    ...
    private void drawSelectedRect(Rect r, Canvas canvas, Paint p) {
        ...
        if (mSelectionMode != SELECTION_HIDDEN && !mSelectionAllday) {
            ...
            if (mNumDays > 1) {
                ...
                //android.util.Log.d("xiao111","mNewEventHintString == " +mNewEventHintString);
    ...
    private void drawHours(Rect r, Canvas canvas, Paint p) {
        ...
        for (int i = 0; i < 24; i++) {
            String time = mHourTimes[i];
            canvas.drawText(time, HOURS_LEFT_MARGIN + 40, y, p);//xiao left time 
            y += mCellHeight + HOUR_GAP;
        }
    }

    private void setupHourTextPaint(Paint p) {
        p.setColor(mCalendarHourLabelColor); //时间小时颜色 xiao  mCalendarHourLabelColor
        p.setTextSize(HOURS_TEXT_SIZE);
        p.setTypeface(Typeface.DEFAULT);
        p.setTextAlign(Align.RIGHT);
        p.setAntiAlias(true);
    ...
    private void drawDayHeader(String dayStr, int day, int cell, Canvas canvas, Paint p) {
        ...
        //if (mNumDays > 1) {//周
            ...
            if(todayIndex == day){
                ...
                mCirclepaint.setColor(mTodayCircleColor);
                mCirclepaint.setStyle(Style.STROKE);
                mCirclepaint.setStrokeWidth(3); 
                canvas.drawCircle(x - 40,y - 23,20,mCirclepaint);
            }
            
            //p.setTypeface(todayIndex == day ? mBold : Typeface.DEFAULT);
            
            canvas.drawText(dateNumStr, x - 40, y - 13, p);
            // Draw day of the week
            x -= p.measureText(" " + dateNumStr);
            p.setTextSize(28);//xiao
            p.setTypeface(Typeface.defaultFromStyle(Typeface.BOLD));
            //android.util.Log.d("xiao111","dayStr == "+dayStr);

            if(dateNumStr.length() == 2){
                canvas.drawText(dayStr, x - 5, y-56, p);
            }else{
                canvas.drawText(dayStr, x - 20, y-56, p);
            }
            
            /*if(mNumDays == 1){
                canvas.drawText(dateNumStr, x - 240, y - 13, p);
                if(dateNumStr.length() == 2){
                    canvas.drawText(dayStr, x - 200, y-56, p);
                }else{
                    canvas.drawText(dayStr, x - 215, y-56, p);
                }
            }*/   
        ...
        /*} else {
            ...
            p.setTextSize(DAY_HEADER_FONT_SIZE);
            p.setTypeface(Typeface.DEFAULT);
            canvas.drawText(dayStr, x, y, p);//xiao

            // Draw day of the month
            x += p.measureText(dayStr) + DAY_HEADER_ONE_DAY_RIGHT_MARGIN;
            p.setTextSize(DATE_HEADER_FONT_SIZE);
            p.setTypeface(todayIndex == day ? mBold : Typeface.DEFAULT);
            android.util.Log.d("xiao111","dateNumStr == "+dateNumStr);
    ...
    private void drawBgColors(Rect r, Canvas canvas, Paint p) {
        ...
        if (mNumDays == 1 && todayIndex == 0) {
            ...
            if (lineY < mViewStartY + mViewHeight) {
                ...
                p.setColor(mFutureBgColor);//xiao days feature grids color
                canvas.drawRect(r, p);
            }
